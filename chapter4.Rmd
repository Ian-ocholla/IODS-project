...
The summary shows the descriptive statistics of each of the variable in the Boston data.  

#### 1.2 Correlation plots between the variables
```{r}
...
#### 1.3 Correlation matrix to show the correlation
```{r}
...
From the correlation plots and tables, the variables _tax_ and _rad_ are highly correlated (corr> 0.8). This is further captured in the visualization of the correlation. 

### 2.0 Data analysis  

#### 2.1 Standardizing the data set 
In scaling, we subtract the column means from the corresponding columns and divide the difference with standard deviation
...
* All the data have been standardized to range between -4 to 10, with **all the variables** having a **mean of zero (0)**. Compared to the original data where the data ranged between 0.00 to 711 with varying mean values
...
Perform K-means using some reasonable number of clusters (>2).
...
Add the clusters into the scaled boston dataset
...
Create training and test dataset from the scaled boston data
...
* which variables are the most influencial linear separators for the clusters?  
